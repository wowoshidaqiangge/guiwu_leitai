<template>
  <el-container>
    <div class="alarmauthority">
      ...
      <!-- 新增联系人按钮 -->
      <div class="addpoint">
        <el-button type="primary" size="small" class="adduser" icon="el-icon-circle-plus-outline" @click="userAddInitBt">新增用户</el-button>
      </div>
      <!-- 新增联系人弹窗 -->
...
export default {
  ...
  methods: {
    ...
    // 查询报警联系人列表
    backUserContactList: function () {
      console.log(this.param)
      back.getTriggerList(this.param).then(function (response) {
    ...
    // 后台进行报警联系人的添加
    backUserContactAdd: function () {
      back.addTriggerPerson(this.param).then(function (response) {
        if (response.errno !== 0) {
    ...
    backUserContactUpdate: function () {
      console.log('修改报警联系人')
      console.log(this.param)
      back.contactMod(this.param).then(function (response) {
        console.log(response)
        if (response.errno !== 1) {
          let obj = {}
    ...
    backUserContactDel: function () {
      console.log('删除报警联系人')
      console.log(this.param)
      back.contactDel(this.param).then(function (response) {
        console.log(response)
        if (response.errno !== 1) {
...
.alarmauthority .alarmauthoritymain {
  padding: 0;
  overflow-x: hidden;
  height: 70%;
  padding-top: 30px;
...
.alarmauthoritymain .el-table__row td:not(.is-hidden):nth-child(1){
  bottom:-1px;
}
</style>
